...
class _LoginScreenState extends State<LoginScreen> {
                    ...
                    Center(
                      child: SizedBox(
                        ...
                        child: ElevatedButton.icon(
                          ...
                          onPressed: () async {
                           NetworkResponse response = await GoogleSignController().signInWithGoogle();
  ...
  onClickLoginButton() async {
    if (_SignInFormKey.currentState!.validate()) {
      NetworkResponse response = await LoginController().loginUser(
        email: emailController.text.trim(),
        password: passwordController.text.trim(),
      );
      if (response.isSuccess) {
        print("Login Success: ${response.responseData}");
        await AuthController.saveData(response.responseData!['id'],
